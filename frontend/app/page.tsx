...
import { ShieldAlert, Users, Map, Lock, Gamepad2, Loader2 } from 'lucide-react';
import api from '../lib/axios';
import TournamentCard from '../components/TournamentCard'; // Assuming you have this component
...
export default function MainDashboard() {
  ...
  useEffect(() => {
    // Fetch tournaments for the dashboard
  ...
  return (
    <div className="min-h-[85vh] text-gray-300 animate-in fade-in duration-500">
      ...
      {/* 🔴 ARENA RULES & GUIDELINES (From F6 Screen) */}
      <div className="bg-[#11141D] border border-red-500/20 rounded-2xl p-5 sm:p-8 mb-10 shadow-lg">
        ...
        <div className="space-y-3">
          <div className="bg-[#0A0C10] border border-gray-800 p-4 rounded-xl flex items-start gap-3">
            ...
            <p className="text-sm"><strong className="text-white">No PC Players Allowed:</strong> Strictly no PC or Emulator players permitted in any match.</p>
          </div>
          
          <div className="bg-[#0A0C10] border border-gray-800 p-4 rounded-xl flex items-start gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-2 flex-shrink-0"></div>
            <p className="text-sm"><strong className="text-white">Mobile Only:</strong> Platform is for mobile players only. Violators will be kicked and entry fee forfeited.</p>
          </div>

          <div className="bg-[#0A0C10] border border-gray-800 p-4 rounded-xl flex items-start gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-2 flex-shrink-0"></div>
            <p className="text-sm"><strong className="text-white">UID Verification:</strong> In-game Free Fire UID must perfectly match your saved Profile UID for payouts.</p>
          </div>

          <div className="bg-[#0A0C10] border border-gray-800 p-4 rounded-xl flex items-start gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-2 flex-shrink-0"></div>
            <p className="text-sm"><strong className="text-white">Zero Tolerance:</strong> Hacks, scripts, glitches, or teaming up will result in a permanent ban.</p>
          </div>

          <div className="bg-[#0A0C10] border border-gray-800 p-4 rounded-xl flex items-start gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-2 flex-shrink-0"></div>
            <p className="text-sm"><strong className="text-white">Timings:</strong> Join exactly at the scheduled time. Admin decisions are final.</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 text-[#00F0FF]">
          ...
          <p className="text-sm font-bold uppercase tracking-widest text-gray-400">Loading Arena...</p>
        </div>
        ...
        <div className="bg-[#11141D] border border-gray-800 rounded-2xl p-12 text-center text-gray-500 flex flex-col items-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">No matches available right now</h3>
          <p>Check back later or ask admins to host a new tournament!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Main map loop for tournaments */}
          {tournaments
          ...
          {/* Fallback Static Card (Just to match your image if no backend data exists yet) */}
          {tournaments.length === 0 && (
            <div className="bg-[#11141D] border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-600 transition-colors">
              <div className="p-5">
                ...
                <h3 className="text-xl font-black text-white mb-2">classic</h3>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-[#0A0C10] border border-gray-800 p-3 rounded-xl text-center">
                    ...
                    <p className="text-yellow-500 font-black text-lg">₹10</p>
                  </div>
                  <div className="bg-[#0A0C10] border border-gray-800 p-3 rounded-xl text-center">
